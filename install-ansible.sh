...
if [ $? -eq 1 ]; then
  ...
  if [ -z $ANSIBLE_DEBUG ]; then
    ...
    apt-get -qq --force-yes install git python-setuptools python-dev libffi-dev libssl-dev python-pip python-crypto -yq > /dev/null 2>&1
  else
    apt-get --force-yes update
    apt-get --force-yes install git python-setuptools python-dev libffi-dev libssl-dev python-pip python-crypto -yq
  fi
